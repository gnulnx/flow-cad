...
This repository is the Flow CAD reusable workbench/runtime. It owns the `flow`
CLI, project bootstrap, project loader, viewer backend/frontend integration,
exporters, cache/report plumbing, reusable geometry helpers, tests, docs, and
bundled reusable agent skills.
...
- `flow-cad` owns reusable runtime/tooling: `flow` CLI, `flow init`, `flow cad
  build`, `flow start`, `flow reload`, viewer service, exporters, cache,
...
- Flow CAD projects own their CAD source. A typical project has
  `flowcad.project.yaml`, `flow/params.py`, `flow/registry.py`,
  `flow/parts/`, `flow/assemblies/`, `flow/validators/`, `docs/`, `exports/`,
  `reports/`, `.flow/`, and project-local `skills/`.
...
- Reusable bundled skills copied by `flow init`: `skills/`
...
Flow CAD runtime/tooling behavior rather than one robot's geometry.
