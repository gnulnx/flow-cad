...
## Upper Adapter Deck To Lower Chassis

Purpose: upper adapter deck bolts to the lower side-frame top rail and carries the wide upper module stack.
...
- Source: `make_upper_wide_center_adapter_deck()` in `src/flow_cad/parts/upper_module.py`
- Nominal bbox: `240.0 W x 256.0 D x 8.0 H mm`

Interface contract:

- Interface type: bolted adapter deck on top of side-frame rails.
- Install direction: Z.
- Bolt-pattern plane: X/Y.
- Deck sits at `P.upper_adapter_deck_z`.
- Upper clamp stack is upper compute bay / over-wheel wing where present / center adapter deck / lower side-frame heat-set inserts.
- The deck must remain within the 256 mm P2S print envelope.
