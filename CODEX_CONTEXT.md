...
- Default equipment shelf: `erb_equipment_shelf_four_way_cable_shallow.step`, a shallow cable-notch shelf with centered notches on all four edges. Older solid, deep side-cable, and shallow left/right side-cable shelf variants remain exported.
- Front/rear panel side attachment: M5 heat-set pilot holes in the panel side rails, 6.1 mm diameter x 24 mm through-cut, at X +/-81 mm, global Y +/-111 mm when assembled, Z 35/105/175/220 mm. These align with the side-panel M5 clearance/counterbore holes.
...
- `erb_lower_chassis_rear_panel.step` (default no-vent rear panel with tapered hollow cable bump-out)
...
- `erb_equipment_shelf_four_way_cable_shallow.step` (default shelf with shallow centered cable notches on all four edges)
- `erb_shelf_spacer_block_55mm.step` (print four copies for the optional third shelf stack)
...
- 2026-05-07 third shelf stack: added `erb_shelf_spacer_block_55mm.step`, a 20 x 50 x 55 mm spacer block with a 4.5 mm through-hole matching the existing M4 shelf-hole pattern. The default assembly now shows four spacer blocks on the upper shelf at X +/-80 mm and Y +/-75 mm, plus a third four-way shallow shelf at Z=183 mm. This gives 55 mm clear height between the upper shelf top and third shelf underside. The first spacer block stays M4-compatible because current side-plate shelf ledges use M4 heat-set insert pilots; changing the stack to M5 would also require changing the shelf holes and side-ledge insert pilots.
...
- 2026-05-06 rear panel change: `erb_lower_chassis_rear_panel.step` is now the default no-vent rear panel and is used in the assembly. It has a 22 mm deep outward tapered bump-out, hollow/open on the chassis interior for cable slack, tapering from 132 x 192 mm at the panel to a blank 104 x 164 mm outer face for Bambu Studio text. The top/bottom border now matches the left/right border at 24 mm. The previous vented rear panel moved to `erb_lower_chassis_rear_panel_vented.step`.
