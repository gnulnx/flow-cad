...
from flow_cad.registry import expected_printable_export_relative_paths
...
def should_include(path: Path, exports_dir: Path = EXPORTS_DIR, active_export_paths: set[Path] | None = None) -> bool:
...
def create_bundle(output_dir: Path, name: str | None = None, active_export_paths: set[Path] | None = None) -> Path:
...
def main() -> int:
    ...
    bundle_path = create_bundle(args.output_dir, args.name, ACTIVE_EXPORT_PATHS)
